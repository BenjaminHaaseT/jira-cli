...
impl Navigator {
    pub fn new(db: Rc<JiraDatabase>) -> Self {
        let homepage: Box<dyn Page> = Box::new(HomePage { db: Rc::clone(&db) });
    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            ...
            Action::CreateEpic => {
                ...
                let new_epic = self.prompts.create_epic.as_ref()();
            ...
            Action::CreateStory { epic_id } => {
                ...
                let new_story = self.prompts.create_story.as_ref()();
                self.db.create_story(new_story, epic_id)?;
            }
            Action::UpdateStoryStatus { story_id } => {
                // prompt the user to update status and persist it in the database
                let updated_status = self.prompts.update_status.as_ref()().ok_or(anyhow!("no story with id: {story_id}"))?;
